...
function LevelSubset(setcelldb, relationdb, generateId) {
  ...
  this.generateId = async function (callback) {
    // to make sure it supports both callback and promise
    return await generateId(callback);
  };
}

LevelSubset.prototype.createSet = (0, _versatileFunction.appendCallback)(async function (cell) {
  const id = await this.generateId();
  await Promise.all([this.setcelldb.putIndexedObject({
    id: id,
    ...cell
  }), this.relationdb.put({
    subject: id
...
LevelSubset.prototype.updateSet = (0, _versatileFunction.appendCallback)(async function (setId, cellInfo) {
  await this.setcelldb.delByPrimaryValue(setId);
  await this.setcelldb.putIndexedObject({
    id: setId,
    ...cellInfo
  });
});
LevelSubset.prototype.deleteSet = (0, _versatileFunction.appendCallback)(async function (setId, failIfSupersetExist = false) {
...
LevelSubset.prototype.getSetById = (0, _versatileFunction.returnPromise)(1, function (id, callback) {
  return this.setcelldb.getByPrimaryValue(id, callback);
});
LevelSubset.prototype.getSetIdsByName = (0, _versatileFunction.returnPromise)(1, function (name, callback) {
  return this.setcelldb.getObjectIdsByIndex('name', name, callback);
}); // -- Getters --

LevelSubset.prototype.getSubsetIds = (0, _versatileFunction.returnPromise)(1, function (mainsetId, callback) {
  this.relationdb.get({
...
LevelSubset.prototype.getSubsetCells = (0, _versatileFunction.appendCallback)(async function (mainsetId) {
  const ids = await this.getSubsetIds(mainsetId);
  return await Promise.all(ids.map(id => this.getSetById(id)));
});
LevelSubset.prototype.getSubsetIdsRecursively = (0, _versatileFunction.appendCallback)(async function (biggestSetId) {
  const self = this;
  const result = [];

  async function findSubset(setId) {
    let ids = await self.getSubsetIds(setId); // delete sets already gotten. (circularly related ones)
...
LevelSubset.prototype.getSubsetCellsRecursively = (0, _versatileFunction.appendCallback)(async function (biggestSetId) {
  const ids = await this.getSubsetIdsRecursively(biggestSetId);
  return await Promise.all(ids.map(id => this.getSetById(id)));
});
LevelSubset.prototype.getSupersetIds = (0, _versatileFunction.appendCallback)(async function (setId) {
  const supersets = await this.getSupersetCells(setId);
  return supersets.map(triple => triple.object);
});
LevelSubset.prototype.getSupersetCells = (0, _versatileFunction.returnPromise)(1, function (setId, callback) {
  this.relationdb.get({
    subject: setId,
    predicate: 'isSubsetOf'
  }, (err, list) => {
...
LevelSubset.prototype.getSupersetIdsRecursively = (0, _versatileFunction.appendCallback)(async function (setId) {
  const self = this;
  const supersets = [];

  async function getSupersets(target) {
    try {
      let list = await new Promise((res, rej) => self.relationdb.get({
        subject: target,
        predicate: 'isSubsetOf'
      }, (err, result) => {
      ...
      if (e.name === 'NotFoundError') return undefined;else throw e;
    }
  }
...
LevelSubset.prototype.isSubsetOfSet = (0, _versatileFunction.appendCallback)(async function (subsetId, supersetId) {
  const self = this;
  let found = false;
...
LevelSubset.prototype.createSetInSet = (0, _versatileFunction.appendCallback)(async function (setCell, biggerSetId) {
  const id = await this.createSet(setCell);
  await this.putSetInSet(id, biggerSetId);
  return id;
});
LevelSubset.prototype.putSetInSet = (0, _versatileFunction.appendCallback)(async function (targetSetId, biggerSetId) {
  ...
  if (isMeaningless) throw new Error('Meaningless shortcut across sets is not allowed. To modify, remove relations first and construct them again.'); // if wrapperSet is a subset (recursively) of target -> circular set
  ...
  await this.relationdb.put({
    subject: targetSetId,
    predicate: 'isSubsetOf',
    object: biggerSetId
  });
});
LevelSubset.prototype.removeSetFromSet = (0, _versatileFunction.returnPromise)(2, function (targetSubsetId, biggerSetId, callback) {
  this.relationdb.del({
    subject: targetSubsetId,
    predicate: 'isSubsetOf',
    object: biggerSetId
  }, callback);
});
var _default = LevelSubset;
