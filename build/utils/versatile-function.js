...
exports.appendCallback = appendCallback;
...
function createCallback(customCallback) {
  let resolve;
  let reject;
  ...
  if (customCallback) {
    callback[promise] = new Promise((res, rej) => {
      resolve = (...result) => {
        res();
        if (result.length > 0) customCallback(null, ...result);else customCallback();
      };
...
function returnPromise(optionalArgumentsHandler, callbackStyleFunction) {
  return function (...args) {
    ...
    if (callbackStyleFunction) {
      switch (typeof optionalArgumentsHandler) {
        ...
        case 'number':
          if (args.length > optionalArgumentsHandler) callback = createCallback(args.pop());else callback = createCallback();
      }
    } else {
    ...
    if (this !== undefined) callbackStyleFunction = callbackStyleFunction.bind(this); // promise will be returned when callbackStyleFunction is already a versatile function.
...
function appendCallback(optionalArgumentsHandler, promiseReturningFunction) {
  return function (...args) {
    let callback;
    let mainFunc;
    ...
    mainFunc(...args).then((...result) => callback(null, ...result)).catch(error => callback(error));
    return callback[promise];
  };
}
//# sourceMappingURL=versatile-function.js.map