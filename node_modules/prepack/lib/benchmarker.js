...
function dump(name, raw, min = raw, compatibility = "browser", outputFilename) {
  ...
  let start = Date.now();
  let realm = (0, _construct_realm.default)({
    serialize: true,
    compatibility
  });
  (0, _globals.default)(realm);
  let serializer = new _index.default(realm);
  let sourceFileCollection = new _types.SourceFileCollection([{
    filePath: name,
    fileContents: raw
  }]);
