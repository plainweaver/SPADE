...
class DataHandler {
  ...
  handleData(data, messageProcessor) {
    this._rawData = Buffer.concat([this._rawData, data]); // the following code parses a message according to the protocol.
    ...
    while (this._rawData.length > 0) {
      ...
      if (this._contentLength >= 0) {
        ...
        if (this._rawData.byteLength >= this._contentLength) {
          ...
          let message = this._rawData.toString("utf8", 0, this._contentLength); // reduce the buffer by the message we got


          this._rawData = this._rawData.slice(this._contentLength); // reset the content length to ensure it is extracted for the next message

          this._contentLength = -1; // process the message
        ...
        if (idx !== -1) {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (pair[0] === "Content-Length") {
              this._contentLength = parseInt(pair[1], 10); // reset the contentlength if it is invalid
