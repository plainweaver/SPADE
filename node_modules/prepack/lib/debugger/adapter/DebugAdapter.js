...
class PrepackDebugSession extends _vscodeDebugadapter.DebugSession {
  ...
  _registerMessageCallbacks() {
    ...
    (0, _invariant.default)(this._adapterChannel !== undefined, "Adapter Channel used before it was created, in debugger."); // Create local copy to ensure external functions don't modify the adapterChannel, satisfy flow.
    ...
    localCopyAdapterChannel.registerChannelEvent(_DebugMessage.DebugMessage.STOPPED_RESPONSE, response => {
      ...
      let message = `${result.reason}: ${result.filePath} ${result.line}:${result.column}`; // Append message if there exists one (for Prepack errors)
    ...
    localCopyAdapterChannel.registerChannelEvent(_DebugMessage.DebugMessage.STEPINTO_RESPONSE, response => {
      let result = response.result;
      (0, _invariant.default)(result.kind === "stepInto");
  ...
  initializeRequest(response, args) {
    ...
    response.body.supportsConfigurationDoneRequest = true; // Respond back to the UI with the configurations. Will add more configurations gradually as needed.
  ...
  configurationDoneRequest(response, args) {
    ...
    if (this._clientID !== _DebuggerConstants.DebuggerConstants.CLI_CLIENTID) {
      ...
      (0, _invariant.default)(this._adapterChannel !== undefined, "Adapter Channel used before it was created, in debugger."); // for all ui except the CLI, autosend the first run request
  ...
  launchRequest(response, args) {
    ...
    let outFilePath = this._generateDebugFilePath("out"); // Set up the communication channel to the debugger.
  ...
  continueRequest(response, args) {
    ...
    (0, _invariant.default)(this._adapterChannel !== undefined, "Adapter Channel used before it was created, in debugger.");
  ...
  setBreakPointsRequest(response, args) {
    ...
    this._adapterChannel.setBreakpoints(response.request_seq, breakpointInfos, dbgResponse => {
      ...
      (0, _invariant.default)(result.kind === "breakpoint-add");
      let breakpoints = [];
  ...
  stackTraceRequest(response, args) {
    ...
    (0, _invariant.default)(this._adapterChannel !== undefined);

    this._adapterChannel.getStackFrames(response.request_seq, dbgResponse => {
      let result = dbgResponse.result;
      (0, _invariant.default)(result.kind === "stackframe");
  ...
  scopesRequest(response, args) {
    ...
    this._adapterChannel.getScopes(response.request_seq, args.frameId, dbgResponse => {
      let result = dbgResponse.result;
      (0, _invariant.default)(result.kind === "scopes");
  ...
  variablesRequest(response, args) {
    ...
    this._adapterChannel.getVariables(response.request_seq, args.variablesReference, dbgResponse => {
      let result = dbgResponse.result;
      (0, _invariant.default)(result.kind === "variables");
  ...
  evaluateRequest(response, args) {
    ...
    this._adapterChannel.evaluate(response.request_seq, args.frameId, args.expression, dbgResponse => {
      ...
      (0, _invariant.default)(evalResult.kind === "evaluate");
      response.body = {
